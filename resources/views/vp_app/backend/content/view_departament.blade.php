        ...
        <div class="row">
                ...
                <div class="col-12 mb-3">
                    <div class="card">
                        <div class="card-body d-flex justify-content-between align-items-center">
                            <h5 class="card-title mb-0">Gestione del Dipartimento</h5>
                            <button type="button" class="btn btn-sm btn-primary"  data-bs-toggle="offcanvas" data-bs-target="#offcanvasEnd" aria-controls="offcanvasEnd">
                                <i class="ti ti-details"></i> Aggiungi reparto
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasEnd">
                <div class="offcanvas-header">
                    <button type="button" class="btn btn-success">NUOVO DIPARTIMENTO</button>
                    <button type="button" class="btn-close text-reset fs-5" data-bs-dismiss="offcanvas"
                            aria-label="Close"></button>
                </div>
                <div class="offcanvas-body">
                    @include('my_app.backend.content.partials_forms.form_departament')
                </div>
            </div>

                @foreach($Departamentet as $Dep)
                    <div class="col-md-6 col-xxl-3">
                        <div class="card border-0 shadow-sm rounded-lg mb-4 card-hover">
                            <div class="position-relative mt-3">
                                <img src="{{ asset($Dep->img_logo) }}" class="card-img-top"  loading="lazy" alt="{{ $Dep->Departament_Lang->emri }}" style="object-fit: contain; width: 100%; height: 150px;">
                                <div class="position-absolute top-0 start-0 bg-dark text-white px-2 py-1 small rounded-end">{{ $Dep->Departament_Lang->emri }}</div>
                            </div>
                            <div class="card-body text-center">
                                <h5 class="card-title fw-bold text-dark">
                                    <i class="ti ti-briefcase me-2"></i>{{ $Dep->Departament_Lang->emri }}
                                </h5>
                                <p class="card-text text-muted small">{{ Str::limit($Dep->Departament_Lang->pershkrimi, 60) }}</p>
                            </div>
                            <div class="card-footer bg-white d-flex justify-content-between">
                                <a href="{{ route('edit_departaments', $Dep->id) }}" class="btn btn-outline-primary btn-sm">
                                    <i class="ti ti-pencil"></i> Modificare
                                </a>
                                <form action="{{ route('delete_departaments', $Dep->id) }}" method="POST" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
